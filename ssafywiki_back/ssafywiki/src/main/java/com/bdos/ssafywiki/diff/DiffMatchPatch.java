...
//    return diff_lineMode(text1, text2, deadline);
     ...
     * @return Number of changes
     */
    public int diff_length(List<Diff> diffs) {
        int length = 0;
        for (Diff aDiff : diffs) {
            switch (aDiff.operation) {
                case INSERT:
                    length += aDiff.text.length();
                    break;
                case DELETE:
                    length -= aDiff.text.length();
                    break;
    ...
    public MergeResult threeWayMerge(String original, String versionA, String versionB){
        LinkedList<Patch> patchesA = patch_make(original, versionA);
        LinkedList<Patch> patchesB = patch_make(original, versionB);

        Object[] resultA = patch_apply(patchesA, original);
        System.out.println(resultA);
        Object[] resultB = patch_apply(patchesB, original);

        boolean conflictDetected = false;
        StringBuilder conflictDetails = new StringBuilder();

        String mergeResult = original;
        for(int i=0; i<resultA.length; i++){
            if(resultA[i].equals(true)){
                mergeResult = (String) resultA[0];
            }else if(resultB[i].equals(true)){
                mergeResult = (String) resultB[0];
            }else{
                // Conflict
                conflictDetected = true;
                conflictDetails.append("Conflict ").append(i).append(" ").append((String)resultA[0]).append("|").append((String)resultB[0]).append("\n");
            }
        }
        return new MergeResult(
                !conflictDetected,
                conflictDetected? "Merge Conflict Detected" : "Merge Successful",
                conflictDetails.toString()
        );
    }

    public String diffToHtml(LinkedList<Diff> diffs) {
        StringBuilder html = new StringBuilder();
        for (Diff diff : diffs) {
            ...
            switch (diff.operation) {
                case INSERT:
                    html.append("<span class='insert'>").append(text).append("</span>");
                    break;
                case DELETE:
                    html.append("<span class='delete'>").append(text).append("</span>");
                    break;
                case EQUAL:
