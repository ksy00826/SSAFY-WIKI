...
public class RedirectDocsService {
    ...
    public RevisionDto.DocsResponse writeDocs(RedirectDocsDto.Post post, User user) {
        ...
        //리다이렉트 문서의 내용(버전) 등록
        String contentString = "#redirect: <LinkTo link=\"https://ssafywiki.info/res/redirect?title="+post.getRedirectTitle()+"&preId="+redirectDocs.getId()+"&preTitle="+redirectDocs.getTitle()+"\">"+post.getRedirectTitle()+"</LinkTo>";
