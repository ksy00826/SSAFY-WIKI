...
public class RedirectDocsService {
    ...
    public RevisionDto.DocsResponse writeDocs(RedirectDocsDto.Post post, User user) {
        ...
        //리다이렉트 문서의 내용(버전) 등록
        String contentString = "[#redirect: " + post.getRedirectTitle() + "](/res/redirect?title="+post.getRedirectTitle()+"&preId="+redirectDocs.getId()+"&preTitle="+redirectDocs.getTitle()+")"; //검색 API 연결 - 수정 필요
