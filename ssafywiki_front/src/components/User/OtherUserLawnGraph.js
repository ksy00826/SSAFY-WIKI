...
const OtherUserLawnGraph = (props) => {
  ...
  const handleClickDay = (e) => {
    ...
    setChildren([]);
    getOtherUserContributeOneDay(date, props.userId).then((result) => {
      //console.log(result);
  ...
  React.useEffect(() => {
    ...
    if (loading) {
      setRows([]);
      getOtherUserContribute(startDate + "T00:00:00", props.userId).then((result) => {
        // //console.log(result);
        const rowss = [];
        result.forEach((week) => {
          const row = [];
...
export default OtherUserLawnGraph;
